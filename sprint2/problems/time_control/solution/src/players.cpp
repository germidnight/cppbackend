...
namespace detail {
     ...
     */
    std::shared_ptr<Player> Players::Add(std::string player_name, std::shared_ptr<model::GameSession> game_session) {
        model::Map* game_map = game_session->GetMap();
        
	/*auto player = players_.emplace_back(std::make_shared<Player>(model::Dog(++next_dog_id_, player_name,
    ...
    JoinGameResult Application::JoinPlayerToGame(model::Map::Id map_id, std::string_view player_name) {
        ...
        auto player = players_.Add(std::string(player_name), game_session);
