...
namespace http_handler {
    ...
    StringResponse APIHandler::ReturnAPIResponse(const StringRequest&& req, std::string req_str,
        ...
        if (req_str.starts_with(command_join_str)) {
            ...
            if ((req.method() == http::verb::get) || (req.method() == http::verb::head)) {
                auto result = CheckAuthentication(req[http::field::authorization], version, keep_alive);
                if (std::holds_alternative<StringResponse>(result)) {
                    return std::get<StringResponse>(result); // Отправили сообщение об ошибке
                }
                return HandlePlayersList(std::get<const players::Player*>(result),
                                            version, keep_alive, (req.method() == http::verb::head));
        ...
        /* ----------------------------------- запрос игрового состояния ----------------------------------- */
            ...
            if ((req.method() == http::verb::get) || (req.method() == http::verb::head)) {
                auto result = CheckAuthentication(req[http::field::authorization],
                                                    version, keep_alive);
                if (std::holds_alternative<StringResponse>(result)) {
                    return std::get<StringResponse>(result); // Отправили сообщение об ошибке
                }
                return HandleGameState(std::get<const players::Player *>(result), version, keep_alive, (req.method() == http::verb::head));
            } else {
                return text_response(http::status::method_not_allowed,
                                     json_loader::MakeErrorString("invalidMethod", "Only GET, HEAD methods is expected"),
                                     0, "GET, HEAD");
    ...
    std::variant<const players::Player*, StringResponse> APIHandler::CheckAuthentication(std::string_view auth_header,
                                                                              unsigned int version,
                                                                              bool keep_alive) {
        const auto text_response = [version, keep_alive](http::status status, std::string_view text, size_t length = 0) {
            return MakeStringResponse(status, text, version, keep_alive, ContentType::JSON, length);
        ...
        const players::Player* found_player = app_.FindPlayerByToken(players::Token(std::move(std::string(bearer_token))));
     ...
     */
    StringResponse APIHandler::HandlePlayersList(const players::Player* found_player,
                                                unsigned int version,
                                                bool keep_alive,
                                                bool head_only) {
        const auto text_response = [version, keep_alive](http::status status, std::string_view text, size_t length = 0,
     ...
     */
    StringResponse APIHandler::HandleGameState(const players::Player *found_player,
                                               unsigned int version,
                                               bool keep_alive,
                                               bool head_only) {
        const auto text_response = [version, keep_alive](http::status status, std::string_view text, size_t length = 0,
