...
namespace detail {
    ...
    const Player* PlayerTokens::FindPlayerByToken(const Token& token) const noexcept {
     ...
     */
    const Player* Players::Add(std::string player_name, model::GameSession* game_session) {
        model::Map* game_map = game_session->GetMap();
        ;
        const Player &player = players_.emplace_back(model::Dog(++next_dog_id_, player_name,
    ...
    JoinGameResult Application::JoinPlayerToGame(model::Map::Id map_id, std::string_view player_name) {
        ...
        const players::Player* player = players_.Add(std::string(player_name), game_session);
