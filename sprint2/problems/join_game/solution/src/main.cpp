...
int main(int argc, const char* argv[]) {
    ...
    try {
        ...
        auto handler = std::make_shared<http_handler::RequestHandler>(game, game_root_dir, api_strand);
        // 5.2. Создаём объект декоратора для логирования
        logging_handler::LoggingRequestHandler logger_handler(std::move(handler));
        /*logging_handler::LoggingRequestHandler logger_handler{[handler](auto&& endpoint, auto&& req, auto&& send) {
        ...
        /*http_server::ServeHttp(ioc, {address, port}, [&logger_handler] (auto&& req, auto&& send,
                    ...
                    logger_handler(std::forward<decltype(req)>(req),
                                    std::forward<decltype(send)>(send),
                                    client_endpoint);
                });*/
